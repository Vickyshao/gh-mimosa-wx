...
(function(win) {
	...
	*/
    ...
    jeDt.weeks = [ "周日", "周一", "周二", "周三", "周四", "周五", "周六" ];
    //节日
    ...
    var config = {
        ...
        zIndex:999,  //弹出层的层级高度
		marks:null, //给日期做标注
        initfun:function(val) {},  //日期构成以后调用
        choosefun:function(val) {},  //选中日期后的回调        
        choosemonth:function(val) {},  //选中月
    ...
    InitDate.prototype = {
        init:function() {
            ...
            if (!QD(Cell)[0]) {
                ...
                document.getElementById("calendar").appendChild(dateDiv);
                //jeDt.html(QD("#calendar")[0], dateDiv);
            }
            try {
        ...
        setHtml:function(opts, self) {
            ...
            //jeDt.html(document.getElementById("calendar"),dateHtmStr);
            ...
            if (jeDt.type(opts.initfun,"function") || opts.initfun != null) {
                opts.initfun(Tms[0]+"-"+Tms[1]);
            }
        },
        ...
        getDateStr:function(y, m, d) {
			...
			function mks(y,m,d){
				...
				if(jeDt.type(opts.marks,"array") && opts.marks.length > 0){
					
					if(contains(opts.marks[0], y+'-'+m+'-'+d)){
						_marks += '<span class="marks_t0"></span>';
					}
					if(contains(opts.marks[1], y+'-'+m+'-'+d)){
						_marks += '<span class="marks_tn"></span>';
					}
					if(contains(opts.marks[2], y+'-'+m+'-'+d)){
						_nows += '<span class="marks_now"></span>';
					}
					
				}
				
				if(_marks.length >　0){
					_marks = '<span class="mark_box">'+ _marks +'</span>';
				}
				
				//console.log(y+'-'+m+'-'+d);
				return _marks + _nows;
				
				//jeDt.type(opts.marks,"array") && opts.marks.length > 0 &&
            ...
            for (var j = startDay; j <= endDay; j++) {
                ...
                dayStr += '<li class="day' + _d + ' ' + current + '" data-y="' + y + '" data-m="' + m + '" data-d="' + j + '">' + isfestival(m + "." + j, j) + mks(y,m,j) + "</li>";
            }
            ...
            if (!isformat) {
                ...
                jeDt.each([ yPre, yNext ], function(i, cls) {
                    jeDt.on(cls, "click", function(ev) {
                        ...
                        if (jeDt.type(opts.choosemonth,"function") || opts.choosemonth != null) {
                            opts.choosemonth(y+"-"+m+"-"+Tms[2]);//getParDate
                        }
                    });
                });
                //切换月
                jeDt.each([ mPre, mNext ], function(i, cls) {
                    jeDt.on(cls, "click", function(ev) {
                        ...
                        that.chooseDays(that, opts, self);
                        if (jeDt.type(opts.choosemonth,"function") || opts.choosemonth != null) {
                            opts.choosemonth(y+"-"+m+"-"+d);//getParDate
                        }
                    });
                });
                //生成定位时分秒
        ...
        chooseYM:function(that, opts, self, Tms) {
            ...
            function clickLiYears(year) {
                jeDt.each(QD(Cell + " .ymdropul li"), function(i, cls) {
                    jeDt.on(cls, "click", function(ev) {
                        ...
                        that.chooseDays(that, opts, self);
                        if (jeDt.type(opts.initfun,"function") || opts.initfun != null) {
                            opts.initfun(Years+"-"+Months+"-"+Tms[2]);//getParDate
                        }
                    });
                });
            }
            ...
            jeDt.on(jedateyy, "click", function() {
                ...
                if (jeDt.type(opts.choosemonth,"function") || opts.choosemonth != null) {
                    opts.choosemonth(yearAttr);//getParDate
                }
            });
            //下拉选择月
            jeDt.on(jedatemm, "click", function() {
                ...
                jeDt.each(QD(Cell + " .ymdropul li"), function(i, cls) {
                    jeDt.on(cls, "click", function(ev) {
                        ...
                        that.chooseDays(that, opts, self);
                        if (jeDt.type(opts.choosemonth,"function") || opts.choosemonth != null) {
                            opts.choosemonth(Years+"-"+Months+"-"+Tms[2]);//getParDate
                        }
                    });
                });
            });
            ...
            jeDt.each(QD(Cell + " .jedaul li"), function(i, cls) {
                jeDt.on(cls, "click", function(ev) {
                    ...
                    if (jeDt.type(opts.choosefun,"function") || opts.choosefun != null) {
                        opts.choosefun(aty+"-"+atm+"-"+atd);//getParDate
                    }
                    that.chooseDays(that, opts, self);
                });
            });
